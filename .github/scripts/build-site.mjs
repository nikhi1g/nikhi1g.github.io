...
fs.mkdirSync(path.join(outputDirectory, 'seminal_papers'), {recursive: true});
fs.copyFileSync('index.html', path.join(outputDirectory, 'index.html'));
fs.copyFileSync('seminal_papers/index.html', path.join(outputDirectory, 'seminal_papers/index.html'));
fs.writeFileSync(path.join(outputDirectory, 'seminal_papers/submissions.json'), `${JSON.stringify(papers, null, 2)}\n`);
